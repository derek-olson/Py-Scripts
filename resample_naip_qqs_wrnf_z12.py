...
workspace = r"I:\Images" #where all the quarter quads are stored
...
#Dummy raster
Dummyrast = arcpy.sa.Raster(r'D:\Region2\ImageBound12') #create this and make sure bigger than extent

# Project area to limit unneccessary processing 
ProjectArea = r'D:\Region2\WhiteRiverAdministrativeForestBuff12.shp'
...
#Get list of images
allRasters = arcpy.ListRasters("*_12_*.tif")
...
for rast in allRasters:
	...
	composite_name = resampleFolder + "/" + rastName +"_30m.img"
	integer_name = resampleFolder + "/" + rastName + "_30m_int8.img"
...
## Only process those rasters that intersect the study area
	if ((xmin >= xminA and xmin <= xmaxA) or (xmax >= xminA and xmax <= xmaxA)) and ((ymin >= yminA and ymin <= ymaxA) or (ymax >= yminA and ymax <= ymaxA)):
		...
		testXmin = (xmin - xminS)%DegradeFactor
		if testXmin!=0:
			changed = True
			xmin = xmin+(DegradeFactor-testXmin)
		testYmin = (ymin - yminS)%DegradeFactor
		if testYmin!=0:
			changed = True
			ymin = ymin + (DegradeFactor-testYmin)
